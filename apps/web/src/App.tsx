...
export function App() {
  ...
  function onYieldSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const form = new FormData(e.currentTarget);
    const errs: Record<string, string> = {};
    if (str(form.get("cropType")).length < 2) setErr("yield-crop", "Enter crop type.", errs);
    const acres = optionalNum(form.get("landSize"));
    if (acres == null || acres <= 0) setErr("yield-acres", "Land size must be positive.", errs);

    runRequest(
      "yield",
      () =>
        callApi("/api/yield-prediction", {
          cropType: str(form.get("cropType")),
          landSize: acres,
          soilType: str(form.get("soilType")) || undefined,
          rainfallExpected: str(form.get("rainfallExpected")) || undefined,
        }),
      () => {
        if (Object.keys(errs).length) {
          setFieldErrors(errs);
  ...
  return (
    <main className="container">
      ...
      <section className="card">
        <h2>Crop Yield Prediction Calculator</h2>
        <form onSubmit={onYieldSubmit} className="form" noValidate>
          <input
            name="cropType"
            placeholder="Crop type (e.g. paddy, tomato)"
            className={fieldErrors["yield-crop"] ? "input-invalid" : ""}
          />
          {fieldErrors["yield-crop"] ? <span className="field-error">{fieldErrors["yield-crop"]}</span> : null}
          <input
            name="landSize"
            type="number"
            min={0}
            step={0.1}
            placeholder="Land size (acres)"
            className={fieldErrors["yield-acres"] ? "input-invalid" : ""}
          />
          {fieldErrors["yield-acres"] ? <span className="field-error">{fieldErrors["yield-acres"]}</span> : null}
          <select name="soilType" defaultValue="">
            <option value="">Select Soil Type (optional)</option>
            <option value="clay">Clay</option>
            <option value="loam">Loam</option>
            <option value="sandy">Sandy</option>
          </select>
          <select name="rainfallExpected" defaultValue="">
            <option value="">Expected Rainfall (optional)</option>
            <option value="low">Low</option>
            <option value="normal">Normal</option>
            <option value="high">High</option>
          </select>
          <button type="submit" disabled={loading === "yield"}>
            {loading === "yield" ? "Calculating..." : "Predict Yield"}
          </button>
        </form>
      </section>
