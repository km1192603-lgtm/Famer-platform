...
app.post("/api/pest-diagnosis", (req, res) => {
  ...
  if (!hasImage) followUpQuestions.push("Upload a clear close-up photo of affected leaves (both sides if possible).");
  if (!growthStage) followUpQuestions.push("Which growth stage is the crop in (nursery, vegetative, flowering, grain-fill)?");
  if (!severity) followUpQuestions.push("How severe is the problem (light / moderate / severe)?");
  ...
  if (!recentRain) followUpQuestions.push("Did it rain or was there heavy dew in the last 48 hours?");
  if (!irrigationType) followUpQuestions.push("What is your main irrigation type (flood, drip, sprinkler, rainfed)?");
...
app.post("/api/weather-risk", async (req, res) => {
  const { district, cropType, cropStage } = req.body as {
    district?: string;
    cropType?: string;
    cropStage?: string;
  };
  ...
  async function fetchOpenWeather() {
    ...
    if (!apiKey) {
      ...
      const hash =
        Array.from(String(district ?? "x")).reduce((acc, ch) => acc + ch.charCodeAt(0), 0) % 1000;
    ...
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(q)}&units=metric&appid=${encodeURIComponent(
      apiKey,
    )}`;
    ...
    const sunlightHours =
      typeof sunrise === "number" && typeof sunset === "number" && sunset > sunrise ? (sunset - sunrise) / 3600 : null;
  ...
  function riskFromWeather(w: any) {
    ...
    const weatherHigh =
      storm || rain >= 20 || temp <= 16 || temp >= 36 || wind >= 70 || (rain >= 10 && hum >= 80);
  ...
  try {
    ...
    const alertLevel = weatherRisk as "low" | "medium" | "high";
    ...
    const followUpQuestions: string[] = [];
    if (!cropStage) followUpQuestions.push("Which crop stage is your crop in right now (sowing/tillering/flowering/harvest prep)?");
    followUpQuestions.push("Is your field prone to standing water after rain (yes/no)?");
    if (pestRisk !== "low") followUpQuestions.push("Did you notice symptoms within the last 3 days (yes/no)?");
...
app.post("/api/cultivation-advice", (req, res) => {
  ...
  const followUpQuestions: string[] = [];
  if (!soilMoisture) followUpQuestions.push("Is topsoil currently dry, moist, or waterlogged?");
...
app.post("/api/market-insights", (req, res) => {
  ...
  const followUpQuestions: string[] = [];
  if (!mandiName) followUpQuestions.push("Which mandi or market do you usually sell at?");
  if (!quantityQuintals) followUpQuestions.push("How many quintals do you plan to sell this week?");
...
app.post("/api/fertilizer-plan", (req, res) => {
  ...
  const followUpQuestions: string[] = [];
  if (!soilTestDone || soilTestDone === "") followUpQuestions.push("Have you done a soil test in the last 12 months (yes/no)?");
...
app.post("/api/schemes-finder", (req, res) => {
  ...
  const followUpQuestions: string[] = [];
  if (!landholdingCategory) followUpQuestions.push("Are you marginal, small, or other farmer category (as per land records)?");
...
app.post("/api/fresher-roadmap", (req, res) => {
  ...
  const equipmentMap: Record<string, { items: string[]; rentalPerAcre: number }> = {
  ...
  const followUpQuestions: string[] = [];
  if (!waterAvailability) followUpQuestions.push("What is your water situation (low/medium/high)?");
  if (!soilType) followUpQuestions.push("Select your soil type (clay/loam/sandy) or do a quick soil test.");
  if (!seedType) followUpQuestions.push("Which seed type do you prefer (local/certified/hybrid)?");
  if (!cropType) followUpQuestions.push("Select crop type for a more accurate equipment + seed suggestion.");
...
app.post("/api/yield-prediction", (req, res) => {
  const { cropType, landSize, soilType, rainfallExpected } = req.body;
  const acres = Number(landSize || 1);
  ...
  const cType = String(cropType || "paddy").toLowerCase();
  let baseYield = 20;
  for (const [key, val] of Object.entries(baseYields)) {
  ...
  if (soilType === "loam") soilMultiplier = 1.1;
  else if (soilType === "sandy") soilMultiplier = 0.9;
  
  let rainMultiplier = 1;
  if (rainfallExpected === "high") rainMultiplier = 1.05;
  else if (rainfallExpected === "low") rainMultiplier = 0.85;

  const estimatedYieldPerAcre = baseYield * soilMultiplier * rainMultiplier;
  const totalYieldQuintals = Math.round(estimatedYieldPerAcre * acres * 10) / 10;
  
  const pricePerQuintal = cType.includes("tomato") ? 1500 : 2500;
  const marketValueEstimate = Math.round(totalYieldQuintals * pricePerQuintal);
  ...
  res.json({
    ...
    marketValueEstimate,
    confidence,
    summary: `Estimated yield for ${acres} acre(s) of ${cropType || "crop"} is ${totalYieldQuintals} quintals, valued approx at ₹${marketValueEstimate.toLocaleString()}.`,
    recommendation: "Ensure timely nutrient application and adequate irrigation to meet this yield target.",
    actionSteps: [
      "Follow recommended NPK dosage for your soil type.",
      "Monitor pests during flowering and grain-filling stages.",
      "Harvest at optimum maturity to prevent field losses.",
    ],
    costEstimate: {
      total: 0,
      currency: "INR",
      breakdown: [],
      note: "Based on historical average yields; actual results depend on field management.",
    },
    followUpQuestions: [
      "Have you planned your fertilizer split doses?",
      "Do you have adequate storage or a buyer ready post-harvest?"
    ]
  });
});
